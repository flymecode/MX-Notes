...
这里我们相当于创建里 a ,ab,abc索引

对于等值的查询 where a = ‘’ ‘’ and  b = ‘’ ‘’ and c = “ ”；是没有顺序先后的，但是我们一定要保证存在索引 a,因为mysql 索引优化器会帮我们进行重新排列索引的顺序，如果我们使用范围查询的话我们将匹配到范围查询列之前的索引。where a > “ “ 也会使用索引。where b > ‘’‘’ and c > ‘’‘’ and a > ‘’“” 也会使用索引，只要 where 保证能使用a这个索引，则走联合索引，对于 where a <> “” and ...这就不能走联合索引。
